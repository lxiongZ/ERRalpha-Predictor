...
from sklearn.metrics import confusion_matrix, accuracy_score, balanced_accuracy_score, matthews_corrcoef, roc_auc_score, precision_recall_curve, auc

import os
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from utils import LoadDataset
from model import GraphTransformerModel, GCNModel
...
def load_model_weights(model, model_name):
    model_path = f'../graph_models/agonist/ros/{model_name}.pth'
...
def save_data(df, pretrain_model_path):
    ...
    desc_folder_path = f'../datasets/agonist_external/descriptors'
    mol2vec_folder_path = f'../datasets/agonist_external/mol2vec'
...
df = pd.read_excel('../datasets/agonist_external/raw/agonist_external.xlsx')
save_data(df, pretrain_model_path)

df_mol2vec = pd.read_csv("../datasets/agonist_external/mol2vec/mol2vec.csv")
X_mol2vec, y_mol2vec = split_data(df_mol2vec)

df_desc = pd.read_csv("../datasets/agonist_external/descriptors/descriptors.csv")
...
# Generate graph data
external_graphs = LoadDataset(root='../datasets/agonist_external', raw_filename='agonist_external.xlsx')
...
svm_external_probs = get_ml_predictions(svm_model, X_mol2vec)
rf_external_probs = get_ml_predictions(rf_model, X_desc)

dl_external_probs = [evaluate_model(model, external_loader) for model in dl_models]

all_external_probs = [svm_external_probs, rf_external_probs] + dl_external_probs

all_external_probs = np.array(all_external_probs).T
consensus_external_probs = np.mean(all_external_probs, axis=1)

threshold = 0.4
predicted = (consensus_external_probs >= threshold).astype(int)
SE, SP, ACC, BA, MCC, AUC, PR_AUC = calculate_metrics(y_mol2vec, predicted, consensus_external_probs) 
...
save_results_to_excel(results, "ERR_alpha_stack/results_agonist.xlsx")
