...
def load_model_weights(model, model_name):
    model_path = f'../graph_models/antagonist/ros/{model_name}.pth'
...
def save_data(df, pretrain_model_path):
    ...
    desc_folder_path = f'../datasets/antagonist_external/descriptors'
    mol2vec_folder_path = f'../datasets/antagonist_external/mol2vec'
...
df = pd.read_excel('../datasets/antagonist_external/raw/antagonist_external.xlsx')
save_data(df, pretrain_model_path)

df_mol2vec = pd.read_csv("../datasets/antagonist_external/mol2vec/mol2vec.csv")
X_mol2vec, y_mol2vec = split_data(df_mol2vec)

df_desc = pd.read_csv("../datasets/antagonist_external/descriptors/descriptors.csv")
...
external_graphs = LoadDataset(root='../datasets/antagonist_external', raw_filename='antagonist_external.xlsx')
...
save_results_to_excel(results, "ERR_alpha_stack/results_antagonist.xlsx")