# ERRα Prediction Web Application - Integrated with Applicability Domain Analysis and Data Retrieval
...
# Import cleaning functions and applicability domain analysis tools
from clean import choose_standardize
from applicability_domain import calculate_applicability_domain
...
def load_database():
    try:
        db_path = "data/label_data_with_inchikey.xlsx"
...
def load_model_weights(model, model_name, model_type):
    model_path = f'../graph_models/{model_type}/ros/{model_name}.pth'
...
def predict_agonist(smiles_list):
    ...
    domain_results = calculate_applicability_domain(cleaned_smiles, "Agonist")
    ...
    # Load ML models
    svm_model = load_ml_model('../ml_final_models/agonist/ros/SVM+mol2vec.joblib')
    rf_model = load_ml_model('../ml_final_models/agonist/ros/RF+descriptors.joblib')
    ...
    # Get domain status
    domain_status = [domain_results[smiles] for smiles in cleaned_smiles]
    
    return predictions, consensus_probs, cleaned_smiles, domain_status
...
def predict_antagonist(smiles_list):
    ...
    # Calculate applicability domain
    domain_results = calculate_applicability_domain(cleaned_smiles, "Antagonist")
    ...
    # Load ML models
    ml_models = [
        ('../ml_final_models/antagonist/rus/lgb+mol2vec.joblib', X_mol2vec),
        ('../ml_final_models/antagonist/rus/RF+mol2vec.joblib', X_mol2vec),
        ('../ml_final_models/antagonist/rus/lgb+descriptors.joblib', X_desc),
        ('../ml_final_models/antagonist/rus/RF+descriptors.joblib', X_desc),
        ('../ml_final_models/antagonist/ros/RF+mol2vec.joblib', X_mol2vec),
        ('../ml_final_models/antagonist/ros/RF+descriptors.joblib', X_desc)
    ]
...
def predict_binder(smiles_list):
    ...
    # Calculate applicability domain
    domain_results = calculate_applicability_domain(cleaned_smiles, "Binder")
...
def main():
    ...
    st.image("Schematic diagram.png", caption="Schematic diagram of ERRα-Predictor", use_column_width=True)
                        ...
                        if mol:  # If SMILES is valid
                            with st.spinner("Predicting..."):
                                if model_type == "Agonist":
                                    predictions, probs, cleaned_smiles, domain_status = predict_agonist([smiles_input])
                                elif model_type == "Antagonist":
                                    predictions, probs, cleaned_smiles, domain_status = predict_antagonist([smiles_input])
                                else:  # Binder
                                    predictions, probs, cleaned_smiles, domain_status = predict_binder([smiles_input])
                            ...
                            st.write(f"Applicability Domain Status: {'Inside AD' if domain_status[0] == 'Inside AD' else 'Outside AD'}")
                            
                            # Show warning based on domain status
                            if domain_status[0] == 'Outside AD':
                                st.warning("⚠️ This molecule is outside the model's applicability domain. Predictions may be unreliable!")
                    ...
                    if smiles_text:
                        ...
                        if smiles_list:
                            with st.spinner("Predicting..."):
                                try:
                                    if model_type == "Agonist":
                                        predictions, probs, cleaned_smiles, domain_status = predict_agonist(smiles_list)
                                    elif model_type == "Antagonist":
                                        predictions, probs, cleaned_smiles, domain_status = predict_antagonist(smiles_list)
                                    else:  # Binder
                                        predictions, probs, cleaned_smiles, domain_status = predict_binder(smiles_list)
                                    ...
                                    results_df = pd.DataFrame({
                                        ...
                                        "Domain Status": ["Inside AD" if s == 'Inside AD' else "Outside AD" for s in domain_status]
                                    })
                                    st.dataframe(results_df)
                                    
                                    # Display number of out-of-domain molecules
                                    out_domain_count = sum(1 for s in domain_status if s == 'Outside AD')
                                    if out_domain_count > 0:
                                        st.warning(f"⚠️ {out_domain_count} molecules are outside the model's applicability domain. These predictions may be unreliable!")
                            ...
                            if st.button("Predict"):
                                ...
                                with st.spinner("Predicting..."):
                                    try:
                                        ...
                                        df["Domain Status"] = ["Inside AD" if s == 'Inside AD' else "Outside AD" for s in domain_status]
    ...
    if app_mode == "Prediction":
        st.sidebar.info(
            ...
            Applicability domain analysis helps assess prediction reliability.
            """
        )
